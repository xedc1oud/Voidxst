...
# === Configuration ===
REPO_URL="https://github.com/xedc1oud/Voidxst.git"
...
# === Distro Detection ===
detect_distro() {
  if [[ -f /etc/void-release ]] || has_cmd xbps-query; then
    echo "void"
    return 0
  fi
...
if [[ "$DISTRO" == "void" ]]; then
  ...
  log_error "This script only supports Void Linux."
  exit 1
fi
...
install_dependencies() {
  ...
  if ! has_cmd git || ! has_cmd make; then
  ...
  local PKGS=(
    ...
    pipewire wireplumber pavucontrol easyeffects ffmpeg playerctl
    ...
    wl-clipboard wlsunset wtype zbar glib pipx zenity inetutils power-profiles-daemon
    python3 libnotify flatpak
    tesseract tesseract-ocr-eng tesseract-ocr-spa tesseract-ocr-jpn
    tesseract-ocr-chi_sim tesseract-ocr-chi_tra tesseract-ocr-kor tesseract-ocr-lat
    noto-fonts-ttf noto-fonts-cjk noto-fonts-emoji nerd-fonts
    matugen gpu-screen-recorder wl-clip-persist mpvpaper
    quickshell adw-gtk3 unzip curl
  )

  log_info "Installing dependencies with xbps..."
  local FILTERED
  # shellcheck disable=SC2207
  FILTERED=($(filter_packages "${PKGS[@]}"))

  if [[ ${#FILTERED[@]} -gt 0 ]]; then
    sudo xbps-install -y "${FILTERED[@]}"
  else
...
configure_services() {
  ...
  # NetworkManager
  if [[ -d "/etc/sv/NetworkManager" && ! -L "$SV_DIR/NetworkManager" ]]; then
    log_info "Enabling NetworkManager..."
    sudo ln -s /etc/sv/NetworkManager "$SV_DIR/"
  fi

  # Bluetooth (in Void it's usually bluetoothd)
  if [[ -d "/etc/sv/bluetoothd" && ! -L "$SV_DIR/bluetoothd" ]]; then
    log_info "Enabling Bluetooth..."
    sudo ln -s /etc/sv/bluetoothd "$SV_DIR/"
  elif [[ -d "/etc/sv/bluetooth" && ! -L "$SV_DIR/bluetooth" ]]; then
    log_info "Enabling Bluetooth..."
    sudo ln -s /etc/sv/bluetooth "$SV_DIR/"
  fi
}
